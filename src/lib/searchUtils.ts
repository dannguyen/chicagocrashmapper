...
export function getSearchTokens(query: string): string[] {
  return query.toUpperCase().split(/\W+/).filter(token =>
    token !== 'AND' && token.replace(/\W+/g, '').trim() !== ''
  );
}
...
export function filterIntersections(intersections: Intersection[], query: string, limit: number = 25): Intersection[] {
